...
.check_tsne_params <- function(nsamples, dims, perplexity, theta, max_iter, verbose, Y_init, stop_lying_iter, mom_switch_iter, momentum, final_momentum, eta, exaggeration_factor, fix)
...
# the format of the input data (e.g., distance matrix or neighbors).
{
    ...
    list(no_dims=dims, perplexity=perplexity, theta=theta, max_iter=max_iter, verbose=verbose,
        ...
        eta=eta, exaggeration_factor=exaggeration_factor, fix=fix)
}
